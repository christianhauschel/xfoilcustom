...
contains
    ...
    subroutine set_airfoil(x_in, y_in, n_in, thickness_max, camber_max) bind(c, name='set_airfoil')
        ...
        real(c_float), intent(out) :: thickness_max, camber_max

        ! Local variables
        !
        real :: area, xble, xbte, xinl, xout, xtmp, yble, ybot, ybte, ytmp, ytop, amax, angtol, thick_max, camb_max
        ...
        call geopar(XB, XBP, YB, YBP, SB, NB, W1, SBLe, CHOrdb, AREab, &
                ...
                & APX2bt, thick_max, camb_max)

        thickness_max = thick_max 
        camber_max = camb_max

        !
        ! Output Geometry Data into File
        open (unit = 99, file="temp_geometry.csv", status="replace")
        write(99,*)"max_thickness,max_camber"
        write(99,20000) thickness_max, camber_max
        20000 format (F10.5,",",F10.5)
        close(99)
        !
